...
// april vim 切换不同时刻以便获取初始化数据
var resolutionType = 1;
var KlineStartDate = null;
var isFrist = true;
...
Datafeeds.UDFCompatibleDatafeed.prototype.getBars = function(symbolInfo, resolution, rangeStartDate, rangeEndDate, onDataCallback, onErrorCallback) {
	// console.log(resolution,'resolution:',resolutionType != resolution)
	// (april add Code , but now not use)
	// if( resolutionType != resolution){
	// 	resolution == "D" && (rangeStartDate = Number.parseInt(new Date().getTime() / 1000) - 365 * 24 * 60 * 60);
	// 	resolution == "1" && (rangeStartDate = Number.parseInt(new Date().getTime() / 1000) - 7 * 24 * 60 * 60);

	// 	resolutionType = resolution;
	// }else{

	// 	KlineStartDate = rangeEndDate;
	// }
